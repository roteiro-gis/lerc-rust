...
use crate::io::Cursor;
use crate::materialize::{BandWriteOrder, BandWriter};
use crate::pixel::{output_value, words_from_padded, AllValid, MaskValidity, Sample, Validity};
use lerc_core::{BlobInfo, Error, Result, Version};
...
pub(crate) fn decode_huffman_into<T: Sample, W: BandWriter<T>>(
    ...
    match mask {
        Some(mask) => decode_huffman_into_with_validity(
            cursor,
            info,
        ...
        None => decode_huffman_into_with_validity(cursor, info, AllValid, delta_encode, out),
    }
}

fn decode_huffman_into_with_validity<T: Sample, W: BandWriter<T>, V: Validity>(
    cursor: &mut Cursor<'_>,
    info: &BlobInfo,
    validity: V,
    delta_encode: bool,
                ...
                for col in 0..width {
                    ...
                    if validity.is_valid(pixel) {
                        ...
                        let decoded = if delta_encode {
                            let mut delta = value;
                            if col > 0 && validity.is_valid(pixel - 1) {
                                delta += prev_value;
                            } else if row > 0 && validity.is_valid(pixel - width) {
...
fn read_huffman_tree(cursor: &mut Cursor<'_>, info: &BlobInfo) -> Result<HuffmanInfo> {
    ...
    if size <= 0 || i0 < 0 || i1 <= i0 {
        return Err(Error::invalid_blob("invalid Huffman table header"));
    }

    let size =
        usize::try_from(size).map_err(|_| Error::invalid_blob("invalid Huffman table header"))?;
    let i0 =
        usize::try_from(i0).map_err(|_| Error::invalid_blob("invalid Huffman table header"))?;
    let i1 =
        usize::try_from(i1).map_err(|_| Error::invalid_blob("invalid Huffman table header"))?;
    let code_length_count = i1
        .checked_sub(i0)
        .ok_or_else(|| Error::invalid_blob("invalid Huffman table header"))?;
        ...
        match info.version {
            ...
            Version::Lerc1(_) => {
                return Err(Error::Internal(
                    "Lerc2 Huffman decoder received metadata for a different format",
                ))
            }
        },
        &mut code_lengths,
    ...
    for i in i0..i1 {
        ...
        {
            return Err(Error::invalid_blob("invalid Huffman code length"));
        }
...
fn read_huffman_symbol(stream: &mut HuffmanStream<'_>, info: &HuffmanInfo) -> Result<u32> {
    ...
    for used_bits in 0..info.max_bits {
        ...
        node = if bit == 0 {
            node.left
                .as_deref()
                .ok_or_else(|| Error::invalid_blob("Huffman decode walked a missing left node"))?
        } else {
            node.right
                .as_deref()
                .ok_or_else(|| Error::invalid_blob("Huffman decode walked a missing right node"))?
        };

        if node.left.is_none() && node.right.is_none() {
            let symbol = node
                .value
                .ok_or_else(|| Error::invalid_blob("Huffman leaf is missing a symbol"))?;
            stream.advance((used_bits + 1) as usize)?;
            return Ok(symbol);
        }
    }

    Err(Error::invalid_blob(
        "Huffman symbol exceeded the configured code width",
    ))
}
...
impl<'a> HuffmanStream<'a> {
    fn peek_bits(&self, num_bits: usize) -> Result<u32> {
        ...
        if num_bits > usize::from(HUFFMAN_CODE_BITS_MAX) {
            return Err(Error::invalid_blob("Huffman bit width exceeds u32"));
        }
    ...
    fn advance(&mut self, bits: usize) -> Result<()> {
        ...
        self.src_ptr = self
            ...
            .ok_or(Error::SizeOverflow("Huffman bitstream pointer"))?;
...
fn read_i32(bytes: &[u8]) -> Result<i32> {
    let bytes = <[u8; 4]>::try_from(bytes)
        .map_err(|_| Error::Internal("Huffman i32 field has the wrong width"))?;
